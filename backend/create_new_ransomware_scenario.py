...
try:
    ...
    phase1 = ScenarioPhase(
        ...
        briefing_text="At 09:20 AM, your phishing campaign has successfully delivered emails to two departments:\n- IT Department (WS-IT-089): 8 emails opened, 2 links clicked\n- Sales Department (WS-SLS-203): 15 emails opened, 5 links clicked\n\nBoth departments have users who clicked the malicious links. You now have initial access attempts on workstations in both departments:\n- **WS-IT-089** (IT): it.admin@corp.local clicked malicious link\n- **WS-SLS-203** (Sales): sales.rep@corp.local clicked malicious link\n\nHowever, you have limited resources and want to focus your efforts on the target that offers the **best chance of establishing a persistent foothold without immediate detection**.\n\nYou've collected initial reconnaissance data on both targets (WS-IT-089 and WS-SLS-203). Your decision: **Which department should you prioritize for establishing persistence?**\n\n**Remember:** Review the artifacts carefully. They contain critical information about security posture, EDR coverage, and user privileges for **both WS-IT-089 and WS-SLS-203** that will determine your success. The artifacts will reveal which target has weaker security controls and presents the better opportunity.",
    ...
    phase2 = ScenarioPhase(
        ...
        briefing_text="You have successfully established initial access on the target workstation. Now you need to create persistence mechanisms to maintain access even if the initial entry point is discovered.\n\nYou've tested three different persistence methods and collected data on their detection and blocking capabilities. Your decision: **Which persistence mechanism should you deploy?**\n\n**Remember:** Review the artifacts carefully. They contain information about detection rates, blocking capabilities, and visibility for each persistence method.",
    ...
    phase3 = ScenarioPhase(
        ...
        briefing_text="You have established persistence on the initial workstation, but you only have standard user privileges. To access sensitive data and systems, you need to escalate privileges.\n\nYou've identified two potential targets for privilege escalation:\n- **File Server FS-01**: Appears to be fully patched\n- **Application Server APP-02**: Appears to have unpatched vulnerabilities\n\nYou've collected vulnerability scan results and reconnaissance data on both targets. Your decision: **Which server should you target for privilege escalation?**\n\n**Remember:** Review the artifacts carefully. They contain vulnerability information, patch status, and exploit availability for **both FS-01 and APP-02** that will determine your success.",
    ...
    phase4 = ScenarioPhase(
        ...
        briefing_text="You have successfully escalated privileges and now have access to multiple servers. You've identified two database servers that contain valuable data:\n- **DB-CUST-01** (Customer Database): Contains customer PII and records\n- **DB-FIN-02** (Financial Records): Contains financial data and records\n\nYou've conducted network mapping and access tests on both targets. Your decision: **Which database server should you target for data access?**\n\n**Remember:** Review the artifacts carefully. They contain network topology information, access logs, and security controls for **both DB-CUST-01 and DB-FIN-02** that will determine your success.",
    ...
    phase5 = ScenarioPhase(
        ...
        briefing_text="You have successfully accessed valuable data from the target database. Now you need to exfiltrate this data before deploying ransomware. You've tested two exfiltration methods and collected data on their detection capabilities.\n\nYour decision: **Which exfiltration method should you use?**\n\n- Option A: Encrypted HTTPS tunnel (slower but may be less detectable)\n- Option B: DNS tunneling (faster but may be more detectable)\n- Option C: Split data across both methods\n\n**Remember:** Review the artifacts carefully. They contain DLP alert analysis, network bandwidth data, and detection test results that will determine which method offers the best balance of speed and stealth.",
